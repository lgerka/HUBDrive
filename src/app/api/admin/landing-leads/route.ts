...
/**
 * Стадии, которые можно поставить: по умолчанию и две, которые уходят
 * в рекламу. Всё остальное было ведением клиента, а для этого нужна CRM —
 * отдельная задача, за которую пока не берёмся. Мёртвая заявка теперь
 * удаляется, а не помечается отказом.
 *
 * В самом типе базы убранные значения остались: удалять значение
 * из перечисления Postgres значит пересоздавать тип целиком, и ради
 * неиспользуемых слов это лишний риск на боевой базе.
 */
