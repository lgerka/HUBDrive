 ...
 *
 * Казахстан оставил за собой перечень изъятий по обязательствам ВТО
 * (Решение Совета ЕЭК №59, в редакции №110 от 23.12.2025): электромобили
 * и последовательные гибриды, коды 8703 80 000 3/4/5/8, — ноль вместо
 * пятнадцати процентов. Обычному гибриду (8703 40–70) ноль положен, только
 * если мощность ДВС не больше 30-минутной мощности электромотора, — это
 * видно лишь по документам конкретной модели, поэтому признак ставит
 * менеджер. Цена льготы — машину нельзя вывозить и перепродавать в другие
 * страны ЕАЭС. Поэтому это выбор менеджера, а не автоматика: решение
 * принимается до сделки, а не калькулятором.
 */
export function canUseWtoRate(powertrain: Powertrain, phevWeakEngine = false): boolean {
    if (powertrain === 'phev') return phevWeakEngine;
 ...
 *
 * Налоговый кодекс 2026, ст. 615 п. 4: «до 2 лет» — 0,25, «от 2 до 3 лет» —
 * средняя ставка, «от 3 лет и выше» — максимальная. Возраст здесь — разница
 * календарных лет: машина прошлого года ещё «до 2 лет», позапрошлого — уже
 * средняя ступень, три года назад и старше — 500 МРП (250 у электромобиля).
 * Раньше граница стояла на год позже, и машина 2023 года в 2026-м платила
 * 50 МРП вместо 500 — почти два миллиона тенге мимо.
 */
...
export interface CalcInput {
    ...
    kzOnly: boolean;
    /**
     * Обычный гибрид, у которого ДВС не мощнее 30-минутной мощности
     * электромотора: тогда и ему положена нулевая ставка ВТО.
     * Каталог его не ставит — там пошлина 15%, чтобы цена не оказалась ниже.
     */
    phevWeakEngine?: boolean;
...
export function calculate(input: CalcInput): CalcResult {
    ...
    const wtoRate = input.kzOnly
        && canUseWtoRate(input.powertrain, input.phevWeakEngine)
        && city?.country === 'KZ';
    ...
    // Второй акциз — по объёму двигателя, 100 ₸ за каждый см³ всего объёма
    // у машин больше 3000 см³ (НК 2026, ст. 537). Гибрид платит по своему ДВС
