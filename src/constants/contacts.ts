...
/**
 * Во сколько мы оцениваем одну заявку — это уходит в Meta как ценность
 * конверсии.
 *
 * Раньше туда передавалась полная цена автомобиля, и Meta считала, что за
 * оставленный телефон мы заработали двадцать пять тысяч долларов. Любой
 * расчёт окупаемости при этом завышен на два порядка, а оптимизация по
 * ценности гонится за дорогими машинами вместо готовых покупателей.
 *
 * Правильная величина — ожидаемая маржа с поправкой на долю заявок,
 * дошедших до сделки. Значение задаётся в META_LEAD_VALUE_USD: когда
 * накопится статистика продаж, его надо уточнить по факту.
 */
export const LEAD_VALUE_USD = Number(process.env.META_LEAD_VALUE_USD) || 100;

/**
 * Во сколько мы оцениваем состоявшуюся сделку — уходит в Meta как ценность
 * покупки.
 *
 * Это не цена автомобиля, а то, что остаётся нам: маржа с продажи. Значение
 * задаётся в META_DEAL_VALUE_USD. Сейчас стоит середина названного владельцем
 * коридора: маржа гуляет от тысячи до трёх, среднее и есть та величина,
 * по которой Meta ищет людей, приносящих деньги, а не просто обращения.
 * Когда накопится статистика продаж, среднее стоит пересчитать по факту.
 */
export const DEAL_VALUE_USD = Number(process.env.META_DEAL_VALUE_USD) || 2000;
